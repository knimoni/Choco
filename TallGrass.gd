...
func _ready():
	get_tree().current_scene.find_node("Choco").connect("player_moving_signal", self, "choco_exiting_grass")
	get_tree().current_scene.find_node("Choco").connect("player_stopped_signal", self, "choco_in_grass")

func choco_exiting_grass():
	player_inside = false
